...
function call_rfc_chimu_block1(){
	...
	call_rfc_centrocosto($soap_client);
}

function call_rfc_centrocosto(&$soap_client){
	
	$RFC_FUNC = "ZPMRFC_DATOS_CCOSTE";
	...
	if ($soap_client->fault) {
		...
		if ($err) {
			...
			// Process the result
			process_result_centrocosto($soap_return);
		}
	}	
}

function process_result_centrocosto($soap_return){
	
	if( !isset($soap_return['item']) || empty($soap_return['item']) ){
		echo "ERROR PROCESANDO RFC CENTRO COSTO ";
		return false;
	}
	
	$data = $soap_return['item'];
	$res  = upsertValesCentroWebFromRFC_full($data);
	if($res)
		echo "EXITO RFC CENTRO COSTO - ".sizeof($data)." elementos afectados<BR>";
    else
		echo "ERROR RFC CENTRO COSTO<BR>";
}
