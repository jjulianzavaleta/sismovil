...
function call_rfc_chimu_block1(){
	...
	call_rfc_choferes($soap_client);
}

function call_rfc_choferes(&$soap_client){
	
	$RFC_FUNC = "ZPMRFC_DATOS_CHOFER";
	...
	if ($soap_client->fault) {
		...
		if ($err) {
			...
			// Process the result
			process_result_chofer($soap_return);
		}
	}	
}

function process_result_chofer($soap_return){
	
	if( !isset($soap_return['item']) || empty($soap_return['item']) ){
		echo "ERROR PROCESANDO RFC CHOFER ";
		return false;
	}
	
	$data = $soap_return['item'];
	$res  = upsertUsuarioWebFromRFC_full($data);
	if($res)
		echo "EXITO RFC CHOFER - ".sizeof($data)." elementos afectados<BR>";
    else
		echo "ERROR RFC CHOFER<BR>";
}
