...
if(isset( $_GET['id']) ){
    ...
    if(!empty($detallepedido)){//Asigno el grupo del´primer producto, con esto sabre si es tipo Paneton o tipo Pavo
        ...
        if(!empty($pedido['fechavalida'])){
            ...
            $fechafinValides    = "26 de noviembre del 2022";
        }
    }
    ...
    if($pedido['serie'] == 1){
        $telefono_zona  = "";
        $direccion_zona = "Av. América Norte # 2213 Urb. Las Quintanas Trujillo - Perú";
    }

    if($pedido['serie'] == 2){
        $telefono_zona  = "Tel: 074 - 232149 / RPM: # 941836625";
        $direccion_zona = "Av. Francisco Cúneo N° 625 Urb. Patazca - Chiclayo.";
    }
...
// Pie de página
	...
	// $this->SetY(-90);
		...
		$this->MultiCell(180,3,'La presente cotización no obliga a Chimú Agropecuaria S.A. a realizar la reserva de la cantidad del producto cotizada.');
		$this->MultiCell(180,3,'El precio se mantendrá vigente hasta el periodo definido en la presente cotización; para reservar éste deberá enviar la OC y emitir el comprobante de pago.');
        $this->MultiCell(180,3,'El vale será canjeable únicamente por pavo entero. Por favor detallar su pedido por ciudad de entrega.');
...
if($grupoProductosCot == 1){//Pavo
	$pdf->MultiCell(180,$cell_height, 'Gracias por contactarte con Chimú Agropecuaria, 72 años de experiencia con nuestro producto "PAVO SAN FERNANDO" nos permite asegurarte la mejor cena navideña.' );
    $pdf->Ln(2);
...
if($grupoProductosCot == 2){//Paneton
    $pdf->MultiCell(180,$cell_height, 'Gracias por contactarte con Chimú Agropecuaria S.A. y preferir la calidad y garantía de nuestros productos.' );
    $pdf->Ln(2);
    ...
    $pdf->Ln(2);
    $pdf->MultiCell(180,$cell_height, 'A continuación, presentamos la cotización para la presente CAMPAÑA NAVIDEÑA 2022:' );
}
...
if($grupoProductosCot == 1){//Pavo
    $pdf->Ln(2);
    $pdf->SetFont('Times','B',8);
    $pdf->MultiCell(180,$cell_height, 'PAVO ENTERO CONGELADO, marinado, c/menudencia, empacado al vacío, el mismo que es beneficiado con la más alta tecnología de exportación e higiene bajo normas ISO 9001, 14001 y HACCP. (Normas que garantizan la calidad, INOCUIDAD y cuidado del medio ambiente).');
}
...
foreach($detallepedido as $detalle){
	...
	$kilosFinal = $kilosTotal + $kilosFinal;    //Cálculo de kilos total proforma
...
$pdf->Cell(180,$cell_height,'Precio válido: Hasta el '.$fechafinValides,0,1,'L');
$pdf->Ln(2);
$pdf->Cell(180,$cell_height,'Forma de pago: '.$modopago,0,1,'L');
$pdf->Ln(2);
$pdf->Cell(180,$cell_height,'Vigencia de Vales: 31 octubre del 2023',0,1,'L');
$pdf->Ln(2);
$pdf->Cell(180,$cell_height,'Lugar de Canje:	FERIA en Prolongación Av. Fátima s/n Sector Encalada (Costado C.C. Real Plaza - Promart)',0,1,'L');

$pdf->Ln(8);
$pdf->SetFont('Times','',8);
$pdf->MultiCell(180,$cell_height, 'Para mayor información, te puedes comunicar con '.$user_name.' al teléfono '.$user_telefonos.' y al correo electrónico '.$user_correo);
$pdf->Ln(2);
$pdf->SetFont('Times','',8);
$pdf->MultiCell(180,$cell_height, 'Agradecemos su gentil atención y nos despedimos seguros de ofrecer el mejor servicio de venta y despacho de producto que ustedes merecen.' );
...
header('Location: pdfGeneratorv2022.php?id='.$_GET['id']);
