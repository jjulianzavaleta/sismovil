...
// Pie de página
	...
	// $this->SetY(-90);
		...
		$this->MultiCell(180,3,'*El Precio Base/Precio descuento no incluye el 18% del I.G.V.');
...
if($grupoProductosCot == 1){//Pavo
	$pdf->MultiCell(180,$cell_height, 'Gracias por contactarte con Chimú Agropecuaria S.A.y preferir la calidad y garantía de nuestro producto “Pavo San Fernando.“' );
    $pdf->Ln(2);
...
if($grupoProductosCot == 2){//Paneton
    ...
    $pdf->Ln(2);
    $pdf->MultiCell(180,$cell_height, 'A continuación, presentamos la cotización para la presente CAMPAÑA NAVIDEÑA 2021:' );
}
...
if($grupoProductosCot == 1){//Pavo
    $pdf->SetFont('Times','BU',8);
    $pdf->Ln(2);
    $pdf->MultiCell(180,$cell_height, 'Entregable al cliente:');
    $pdf->Ln(2);
    $pdf->SetFont('Times','B',8);
    $pdf->MultiCell(180,$cell_height, 'PAVO ENTERO CONGELADO, marinado, c/menudencia, empacado al vacío, el mismo que es beneficiado con la más alta tecnología de exportación e higiene bajo normas ISO: 9001, 14001, HACCP, 22000 y BPM.');
}
...
if(file_exists('anexos_formato_2021/certificaciones.jpg')){
    $pdf->Image('anexos_formato_2021/certificaciones.jpg',20,null, 160);
   //$pdf->Cell(20,5,'',0,0,'L');
}
...
$pdf->Cell(180,$cell_height,'• Lugares de Canje: Tiendas de Chimú Agropecuaria S.A.',0,1,'L');

$pdf->SetFont('Times','',8);
$pdf->Cell(180,$cell_height,'• Ubicación:',0,1,'L');
$pdf->Cell(180,$cell_height,'               •  Av. América Norte # 2213 Urb. Las Quintanas',0,1,'L');
$pdf->Cell(180,$cell_height,'               •  Av. Larco # 1357 Urb. La Merced',0,1,'L');
$pdf->Cell(180,$cell_height,'               •  Av. Prolongación César Vallejo Mza E lote 9 Urb. Ingeniería (costado Auditorio Univ. UPAO)',0,1,'L');
$pdf->Ln(3);

$pdf->Ln(2);
$pdf->SetFont('Times','',8);
$pdf->MultiCell(180,$cell_height, 'Para mayor información, te puedes comunicar con '.$user_name.' al teléfono '.$user_telefonos.' o escribir a '.$user_correo);
$pdf->Ln(2);
$pdf->SetFont('Times','',8);
...
header('Location: pdfGeneratorv2021.php?id='.$_GET['id']);
