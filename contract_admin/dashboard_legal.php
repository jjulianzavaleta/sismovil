...
$cant_vigentes          = getContratosByEstados(array(4),date("Y"),0,true);
$cant_concluido         = getContratosByEstados(array(5),date("Y"));
$cant_proceso           = getContratosByEstados(array(0,0.3,0.6,0.8,1,2,3),date("Y"));
$cant_anulado           = getContratosAnulados(date("Y"));
$porvencer_15diasOmenos = getContratosPorVencer(15,true);
$porvencer_30diasOmenos = getContratosPorVencer(30,true);

$data_por_estado1        = getRegistradosbyMonth(date("Y"),array(0,0.3,0.6,0.8,1,2,3,4,5),false,false);//registrados
$data_por_estado2        = getRegistradosbyMonth(date("Y"),array(5),true);//concluidos
$data_por_estado3        = getRegistradosbyMonth(date("Y"),array(0,0.3,0.6,0.8,1,2,3,4,5),false,false,true);//anulados
$max_value_por_estado    = max(max($data_por_estado1),max($data_por_estado2),max($data_por_estado3))+5;
$data_by_area			 = getTop3AreasSolCreadas(date("Y"),3);
$max_value_por_area		 = cal_max($data_by_area)+5;
			...
			<div class="col-sm-12 mb-4">
				<div class="card-group">
					...
					<div class="card col-lg-2 col-md-6 no-padding no-shadow">
						...
						<div class="card-footer">
							<small class="text-light">
								<a href="../contract_reportes/index.php?optSelec=1&chk_registrado=1&chk_pendelaboracion=1&chk_pendaprobacionusuario=1&chk_colectarfirmas=1&chk_val_legal_acepta=1&chk_val_jef_log=1&chk_val_jefarea=1&fechaIni=<?=date("Y")?>-01-01&fechaFin=<?=date("Y")?>-12-31">Ver detalle <i class="fa fa-arrow-circle-right"></i></a>
							</small>
						</div>
					</div>
					<div class="card col-lg-2 col-md-6 no-padding no-shadow">
					...
					<div class="card col-lg-2 col-md-6 no-padding no-shadow">
						<div class="card-body bg-flat-color-4">
							...
							<small class="text-light text-uppercase font-weight-bold">Vence en ~15 días</small>
							<div class="progress progress-xs mt-3 mb-0 bg-light" style="width: 40%; height: 5px;"></div>
						</div>
						<div class="card-footer">
							<small class="text-light">
			...
			<div class="col-xl-6">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-4">
                                <h4 class="card-title mb-0">Registrado por área</h4>
                                <div class="small text-muted">Año 2019</div>
                            </div>
                        </div>
                        <!--/.row-->
                        <div class="chart-wrapper mt-4">
                            <canvas id="test" style="height: 250px;"></canvas>
                        </div>
...
function random (min, max) {
  return Math.floor(Math.random() * (max - min + 1) + min)
   ...
   var data1 = [
	...
	//Traffic Chart
    var ctx1 = document.getElementById( "test" );
    //ctx.height = 200;
    ...
    var myChart1 = new Chart( ctx1, {
        ...
        data: {
            ...
            datasets: [
			    ...
			    $colores = array("#4dbd74","#63c2de","#f86c6");				
				$keys = array_keys($data_by_area);
				foreach($keys as $key){
					echo "{";
					echo "label: '".($key=="LOGISTICA"?"LOGÍSTICA":$key)."',";
					echo "backgroundColor: convertHex(brandInfo, 10),";
					echo "borderColor: '".$colores[$index]."',";
					echo "pointHoverBackgroundColor: '".$colores[$index]."',";
					echo "borderWidth: 2,";
            ...
            scales: {
                ...
                yAxes: [ {
                      ticks: {
                        beginAtZero: true,
                        maxTicksLimit: 5,
                        stepSize: Math.ceil(<?=$max_value_por_area?> / 5),
                        max: <?=$max_value_por_area?>
                      },
                      gridLines: {
                        display: true
                      }
